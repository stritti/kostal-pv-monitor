...
class kostal_modbusquery:
    def __init__(self):
        ...
        self.inverter_ip = "192.168.178.60"
...
if __name__ == "__main__":
    ...
    LeftSidePowerGeneration = round(
        (KostalVal["Power DC1"] + KostalVal["Power DC2"]), 1
    )
    print("Left Side Raw Power Generation of Panles :", LeftSidePowerGeneration)
    BatteryCharge = round(
        KostalVal["Battery voltage"]
        * KostalVal["Actual battery charge -minus or discharge -plus current"],
        1,
    )
    ...
    TotalHomeconsumption = round(
        (
            ...
            + KostalVal["Home own consumption from PV"]
        ),
        1,
    )
    PowertoGrid = round(
        KostalVal["Inverter Generation Power (actual)"] - TotalHomeconsumption, 1
    )
